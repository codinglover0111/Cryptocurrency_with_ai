...
AGENT_PROMPTS_TABLE = sa.Table(
    "agent_prompts",
    SCHEMA_METADATA,
    sa.Column("id", sa.Integer, primary_key=True, autoincrement=True),
    sa.Column("agent_type", sa.String(32), unique=True, nullable=False),
...
class TradeStore:
    ...
    # -------------------------------
    # Agent Prompts (DB only)
    # -------------------------------
    def save_agent_prompt(self, agent_type: str, prompt_template: str) -> bool:
        """에이전트 프롬프트를 DB에 저장합니다 (upsert).

        Args:
            agent_type: 에이전트 타입 (예: 'indicator', 'pattern', 'trend', 'decision')
        ...
        try:
            ...
            with self._engine.connect() as conn:
                # 기존 값 확인
                check_sql = sa.text(
                    "SELECT id FROM agent_prompts WHERE agent_type = :agent_type"
                )
                result = conn.execute(check_sql, {"agent_type": agent_type}).fetchone()

                if result:
                    # UPDATE
                    update_sql = sa.text(
                        "UPDATE agent_prompts SET prompt_template = :prompt_template, updated_at = :updated_at "
                    ...
                    # INSERT
                    insert_sql = sa.text(
                        "INSERT INTO agent_prompts (agent_type, prompt_template, updated_at) "
                        "VALUES (:agent_type, :prompt_template, :updated_at)"
                    )
                    conn.execute(
                        insert_sql,
                        {
    ...
    def get_agent_prompt(self, agent_type: str) -> Optional[str]:
        """에이전트 프롬프트를 DB에서 읽어옵니다.

        Args:
            agent_type: 에이전트 타입 (예: 'indicator', 'pattern', 'trend', 'decision')

        Returns:
            프롬프트 템플릿 텍스트 또는 None
        """
        if self._engine is None:
            return None
        try:
            with self._engine.connect() as conn:
                sql = sa.text(
                    "SELECT prompt_template FROM agent_prompts WHERE agent_type = :agent_type"
                )
                result = conn.execute(sql, {"agent_type": agent_type}).fetchone()
                if result:
                    return result[0]
            return None
        except Exception as e:
            print(f"Error reading agent prompt: {e}")
            return None

    def get_all_agent_prompts(self) -> Dict[str, Any]:
        """모든 에이전트 프롬프트를 딕셔너리로 반환합니다.

        Returns:
            {agent_type: {"prompt_template": str, "updated_at": str}, ...}
        """
        if self._engine is None:
            return {}
        try:
            with self._engine.connect() as conn:
                sql = sa.text(
                    "SELECT agent_type, prompt_template, updated_at FROM agent_prompts"
                )
                rows = conn.execute(sql).fetchall()
                result = {}
                for row in rows:
    ...
    def delete_agent_prompt(self, agent_type: str) -> bool:
        """에이전트 프롬프트를 DB에서 삭제합니다.

        Args:
            agent_type: 삭제할 에이전트 타입
        ...
        try:
            with self._engine.connect() as conn:
                sql = sa.text(
                    "DELETE FROM agent_prompts WHERE agent_type = :agent_type"
                )
                conn.execute(sql, {"agent_type": agent_type})
                conn.commit()
            return True
        except Exception as e:
            print(f"Error deleting agent prompt: {e}")
            return False

    def save_agent_prompts_bulk(self, prompts: Dict[str, str]) -> bool:
        """여러 에이전트 프롬프트를 한 트랜잭션에서 일괄 저장합니다 (upsert).

        Args:
            prompts: {agent_type: prompt_template, ...} 형태의 딕셔너리

        Returns:
            성공 여부
        """
        if self._engine is None or not prompts:
            return False
        try:
            now = dt.datetime.utcnow()
            with self._engine.connect() as conn:
                for agent_type, prompt_template in prompts.items():
                    # 기존 값 확인
                    check_sql = sa.text(
                        "SELECT id FROM agent_prompts WHERE agent_type = :agent_type"
                    )
                    result = conn.execute(
                        check_sql, {"agent_type": agent_type}
                    ).fetchone()
            ...
            print(f"Error saving agent prompts bulk: {e}")
            return False
