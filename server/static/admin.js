...
function showSection(sectionId) {
  ...
  const titles = {
    ...
    prompts: {
      title: "프롬프트 설정",
      subtitle: "AI 에이전트별 프롬프트를 커스터마이즈하세요",
    },
  };
  ...
  // Load prompts when prompts section is shown
...
function setupEventListeners() {
  ...
  // Prompts
  const promptAgentSelect = el("prompt-agent-select");
  const savePromptBtn = el("save-prompt");
  const resetPromptBtn = el("reset-prompt");
  const resetAllPromptsBtn = el("reset-all-prompts");
  if (promptAgentSelect)
    promptAgentSelect.addEventListener("change", handlePromptAgentChange);
  if (savePromptBtn)
    savePromptBtn.addEventListener("click", savePrompt);
  if (resetPromptBtn)
    resetPromptBtn.addEventListener("click", resetCurrentPrompt);
  if (resetAllPromptsBtn)
    resetAllPromptsBtn.addEventListener("click", resetAllPrompts);
...
async function loadPrompts() {
  try {
    ...
    // 현재 선택된 에이전트의 프롬프트 표시
    displayCurrentPrompt();
  } catch (err) {
    console.error("프롬프트 로드 실패:", err);
    const hint = el("prompts-hint");
    if (hint) hint.textContent = "✗ 프롬프트 로드 실패";
  }
}

function displayCurrentPrompt() {
  const agentType = adminState.currentPromptAgent;
  const promptData = adminState.prompts[agentType];

  const editor = el("prompt-editor");
  const sourceBadge = el("prompt-source-badge");
  const updatedEl = el("prompt-updated");
  const variablesHint = el("prompt-variables-hint");

  if (!promptData) {
    if (editor) editor.value = "";
    if (sourceBadge) {
      sourceBadge.textContent = "로드 실패";
      sourceBadge.className = "status-badge status-badge--inactive";
    }
    if (updatedEl) updatedEl.textContent = "";
    if (variablesHint) variablesHint.textContent = "사용 가능한 변수: 없음";
    return;
  }

  // 프롬프트 텍스트 설정
  if (editor) {
    editor.value = promptData.prompt || "";
  }

  // 소스 배지 업데이트
  if (sourceBadge) {
    if (promptData.source === "db") {
      sourceBadge.textContent = "DB 저장됨";
      sourceBadge.className = "status-badge status-badge--active";
    } else {
      sourceBadge.textContent = "기본값";
      sourceBadge.className = "status-badge status-badge--inactive";
    }
  }
  ...
  if (updatedEl) {
    if (promptData.updated_at) {
      updatedEl.textContent = `마지막 수정: ${formatTime(promptData.updated_at)}`;
    } else {
  ...
  if (variablesHint) {
    ...
    if (variables.length > 0) {
      variablesHint.innerHTML = `사용 가능한 변수: ${variables
        .map((v) => `<code>{${escapeHtml(v)}}</code>`)
        .join(", ")}`;
    } else {
      variablesHint.textContent = "사용 가능한 변수: 없음";
    }
  }
}

function handlePromptAgentChange(e) {
  adminState.currentPromptAgent = e.target.value;
  displayCurrentPrompt();
}
...
async function savePrompt() {
  ...
  const editor = el("prompt-editor");
  const agentType = adminState.currentPromptAgent;
  const promptTemplate = editor?.value?.trim();

  if (!promptTemplate) {
    if (hint) hint.textContent = "⚠ 프롬프트가 비어있습니다";
    return;
  }
  ...
  try {
    ...
    if (hint) hint.textContent = `✗ 저장 실패: ${err.message}`;
  }
}
...
async function resetCurrentPrompt() {
  const agentType = adminState.currentPromptAgent;
  const promptData = adminState.prompts[agentType];
  const agentLabel = promptData?.label || agentType;

  if (
    !confirm(
      `${agentLabel}의 프롬프트를 기본값으로 초기화하시겠습니까?\n\nDB에 저장된 커스텀 프롬프트가 삭제됩니다.`
    )
  ) {
    return;
  }
  ...
  try {
    if (hint) hint.textContent = "초기화 중...";

    await postJSON(`/admin/prompts/reset/${agentType}`, {});
    ...
    if (hint) hint.textContent = `✗ 초기화 실패: ${err.message}`;
  }
}
...
async function resetAllPrompts() {
  if (
    !confirm(
      "모든 에이전트의 프롬프트를 기본값으로 초기화하시겠습니까?\n\nDB에 저장된 모든 커스텀 프롬프트가 삭제됩니다."
    )
  ) {
    return;
  }
  ...
  try {
    if (hint) hint.textContent = "전체 초기화 중...";

    await postJSON("/admin/prompts/reset-all", {});

    if (hint) hint.textContent = "✓ 전체 기본값으로 초기화됨";
    setTimeout(() => {
      if (hint && hint.textContent === "✓ 전체 기본값으로 초기화됨")
        hint.textContent = "";
    }, 3000);
    ...
    console.error("전체 프롬프트 초기화 실패:", err);
    if (hint) hint.textContent = `✗ 초기화 실패: ${err.message}`;
  }
}
