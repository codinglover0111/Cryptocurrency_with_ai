...
- **Supabase Auth 기반 JWT**: 이메일/비밀번호 기본, OAuth 선택적 활성화.
- **흐름**
  - Next.js 프런트엔드가 Supabase Auth로 로그인/세션을 관리한다.
  - Next.js API 라우트가 세션을 검증한 뒤 FastAPI로 프록시하며 `Authorization: Bearer <supabase_jwt>` 헤더를 전달한다.
  - FastAPI가 Supabase JWKS로 서명/만료를 재검증하고 `role` 클레임(`app_metadata.role` → `user_metadata.role` → 기본 `user`)을 추출한다.
- **역할**
  - `admin`: 에이전트 LLM 설정, 스케줄러 주기, 사용자 관리, 심볼 관리, 포지션 일괄 청산
  - `user`: 대시보드/저널 조회, 자동매매 상태 확인
- **저장소**: Supabase Auth/프로필이 단일 소스이며, 필요 시 서비스 롤 키로 프로필을 조회/캐시한다.
- 세션/IP 차단 기반 레거시 로그인 경로는 제거되었으며 Supabase Auth만 사용한다.
  - IP 차단 환경변수(`MAX_LOGIN_ATTEMPTS`, `LOGIN_ATTEMPT_WINDOW_MINUTES`)는 Supabase 로그인 흐름에서는 사용하지 않는다.

---

## 6. 웹 UI (`app/web/`, `templates/`, `static/`, `frontend/`)

- **Next.js 프런트엔드**: App Router 기반, Supabase Auth로 로그인/세션 관리, API 라우트가 FastAPI로 프록시한다. 공개 대시보드는 SSR, 관리자 화면은 보호된 라우트로 유지한다.
- **기존 관리자/사용자 페이지(레거시 템플릿)**:
  - 에이전트별 모델/파라미터 변경
...
- **사용자 페이지(레거시)**:
  - 잔고, 포지션, 최근 거래, 저널
  - 현재 전략/시장 레짐 설명
- Next.js가 FastAPI 엔드포인트(`/admin/*`, `/user/*`)를 그대로 사용하므로 스키마 변경 시 프런트엔드와 동시 반영해야 한다.
