...
QuantAgent의 실행 계층을 구성하는 핵심 모듈들이 모여 있습니다. 각 하위 폴더의 세부 책임은 해당 폴더의 `agents.md`에서 더 자세히 설명합니다. 인증은 Supabase JWT(Supabase Auth)로 일원화되고, Next.js API 라우트가 FastAPI로 프록시합니다.

## 하위 폴더 개요

- `agents/`: Indicator/Pattern/Trend/Decision 에이전트와 프롬프트·스키마 정의
- `auth/`: Supabase JWT 검증 기반 권한 부여(`require_user`/`require_admin`), 역할 추출, API 키 암호화 키 관리
...
- **Supabase 인증 전환**: 세션 기반 로그인 제거, Supabase JWT + Next.js 프록시로 통합 (`app/auth/`, `webapp.py`)
...
- **런타임 설정 DB 저장**: JSON 파일 대신 DB에 저장, 자동 마이그레이션 지원 (`app/config/`, `utils/storage.py`)
- **Docker uv 지원**: Dockerfile에서 `uv pip install`로 패키지 설치 속도 개선

## 유지보수 메모

- 런타임 설정은 DB(`runtime_config` 테이블)에 저장됩니다. DB 연결 실패 시 `app/config/runtime_config.json`으로 폴백됩니다.
- 기존 JSON 파일이 있으면 앱 시작 시 DB로 자동 마이그레이션됩니다.
...
- 스케줄러 상태, 공유 분석 결과, 런타임 설정은 `utils/storage.py`의 테이블에 저장됩니다.
- 인증/권한은 Supabase JWT로 처리하므로 모든 라우트가 `require_user`/`require_admin` 의존성을 거쳐야 합니다.
