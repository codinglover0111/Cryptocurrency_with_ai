...
def _build_prompt(deps: AutomationDependencies, ctx: PromptContext) -> str:
    ...
    prompt = (
        ...
        "당신은 레버리지 20배에서 50배 정도를 사용하여 하루에 있는 변동성을 통해 이익을 최대화하는 트레이더입니다.\n"
        "잃는 것을 무서워 하지마세요. 언제나 다음이 있습니다.\n"
        "[TRADING_TIPS]\n"
        """# 저항선이란

- 저항선이란  
  가격이 상승하다가 일정 지점에서 더 이상 오르지 못하고 하락세로 전환되는 구간을 의미합니다. 이 지점을 '저항선'이라고 합니다.  
- 저항선의 발생 이유  
  앞서 고점에서 많은 거래가 이뤄진 경우, 그 가격대에 물려 있는 투자자들이 본전 탈출을 위해 다시 가격이 그 지점에 오르면 매도 물량을 쏟아냅니다. 이로 인해 상승세가 저지되고 저항이 생깁니다. 또한 10,000원, 100,000원 등 상징적인 가격대나 이전 고점 부근에서도 투자자들의 수익 실현 심리로 저항이 발생할 수 있습니다.  
- 저항선의 강화  
  여러 번 저항이 발생한 자리에서는 신규 매수세가 줄고 기존 매수자들이 수익을 실현하는 경향이 강해집니다.  
- 저항선 돌파와 역할 변화  
  강한 저항선이 거래량과 함께 돌파될 경우, 세력(큰 자금의 투자자)의 개입일 가능성이 높으며, 돌파된 저항선은 이후 '지지선'으로 바뀌기도 합니다. 이를 '지지리테스트'라 부르기도 하며, 반대로 지지선을 하락 돌파하면 그 자리가 저항선으로 바뀝니다.  
- 매매 팁  
  저항선을 기준으로 매도를 고려하며, 저항선을 강하게 돌파하면 이탈 손절이나 추가 매수 전략을 세울 수 있습니다. 단, 저항선은 무조건적인 반전이 보장되진 않으니 주의해야 합니다.\n
"""
        """# 피보나치 되돌림이란
피보나치 되돌림(Fibonacci Retracement)은 기술적 분석 도구 중 하나로, 주가나 기타 자산 가격이 큰 폭으로 상승하거나 하락한 후 일정 비율만큼 되돌아오는 지점을 예측하는 데 사용됩니다. 이 도구는 되돌림 구간에서 주요한 지지선과 저항선을 설정하여 매수 또는 매도 시점을 판단하는 데 도움을 줍니다.

주요 피보나치 되돌림은 고점과 저점 사이에서 비율은 23.6%, 38.2%, 50%, 61.8%, 78.6%이며, 이 비율들은 피보나치 수열에서 도출되었습니다. 예를 들어, 가격이 100에서 150으로 상승했다면, 38.2% 되돌림은 119.1, 50%는 125, 61.8%는 130.9 수준에서 되돌림이 일어날 가능성이 높아 이를 지지선으로 활용할 수 있습니다. 되돌림 비율은 가격이 추세를 일시 조정하다가 다시 원래 방향으로 재개할 때 중요한 기준점이 됩니다.

피보나치 되돌림은 상승 추세에서는 조정 후 지지선, 하락 추세에서는 저항선 역할을 하며, 트레이더들이 매매 진입과 손절선 설정 등에 활용합니다. 또한, 황금비 61.8%는 강력한 반전 구간으로 여겨집니다.

요약하자면, 피보나치 되돌림은 가격이 일정 비율로 되돌아오는 경향을 이용해 중요한 지지와 저항 수준을 예측하는 기술적 분석 기법입니다.\n"""
        """# RSI 다이버전스 및 거래량 활용법 – TRADING TIPS

- RSI 다이버전스와 거래량의 관계
  - RSI 다이버전스는 '추세 반전 신호'이지만, 거래량과 함께 보면 신호의 신뢰도를 극대화할 수 있음
  - 고점 돌파 이후 거래량이 감소할 때: RSI 다이버전스 신호와 함께 거래량이 줄어들면 매수세가 약화, 매도세가 강해져 실제 하락 전환 가능성↑
  - 저점 갱신 이후 거래량 감소: RSI 상승 다이버전스 신호 + 거래량 감소는 추가 하락세의 약화와 이후 반전 상승 가능성 커짐

- 실전 적용 팁
  - 거래량 증가 동반 여부 확인: RSI 다이버전스 신호가 있을 때 고점 돌파/저점 갱신 시 거래량이 크게 증가한다면 ‘가짜 신호’ 가능성이 있으니 주의
  - 거래량 급감 구간에 집중: RSI 다이버전스+거래량 감소가 동시에 나타나면 실제 매수/매도세가 힘을 잃고 반전 타이밍이 가까워지므로 집중 관찰
  - 거래량과 캔들 형태 동시 분석: 거래량 감소 속에서 음봉/양봉 전환 패턴이 나오는지 확인 → 신뢰도 UP
  - 비트코인 등 변동성 큰 종목에서 강하게 적용: 다이버전스+거래량 감소 패턴은 변동성이 큰 코인·개별주에서 더 효과적

- 예시 전략
  - 상승추세에서 고점 돌파 시 RSI 하락 다이버전스+거래량 감소 = ‘매도포지션’ 고려
  - 하락추세에서 저점 갱신 시 RSI 상승 다이버전스+거래량 감소 = ‘매수포지션’ 고려\n"""
        "[/TRADING_TIPS]\n"
...
def _execute_trade(
    ...
    total_usdt = _safe_float(balance_total)
    free_usdt = _safe_float(balance_free)
    if total_usdt <= 0 and free_usdt > 0:
    ...
    if total_usdt <= 0:
        _record_skip(
            ...
            reason_text="USDT 잔고가 부족하여 주문을 생성할 수 없습니다.",
        )
        return
    ...
    if max_alloc_env is not None:
        try:
            alloc_percent = float(max_alloc_env)
        except Exception:
            alloc_percent = float(deps.per_symbol_alloc_pct or 20.0)
    else:
        alloc_percent = float(deps.per_symbol_alloc_pct or 20.0)
    ...
    if alloc_percent <= 0:
        _record_skip(
            deps,
            reason="invalid_allocation_percent",
            decision=decision,
            meta={
                "alloc_percent": alloc_percent,
                "alloc_env": max_alloc_env,
            },
            reason_text="심볼당 할당 비율이 0% 이하입니다.",
        )
        return
    ...
    if entry_price <= 0:
        _record_skip(
            ...
            reason_text="유효한 진입가를 확인할 수 없습니다.",
        )
        return
    ...
    if not positions_same_symbol:
        try:
            positions_same_symbol = (
                deps.bybit.get_positions_by_symbol(deps.contract_symbol) or []
            )
        except Exception:
            positions_same_symbol = []

    last_price_fallback = _safe_float(ctx.current_price, entry_price)
    ...
    quantity = order_notional / float(entry_price)
    if quantity <= 0:
        _record_skip(
            deps,
            ...
            reason_text="계산된 주문 수량이 0 이하입니다.",
        )
        return
