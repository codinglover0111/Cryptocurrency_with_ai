...
def _execute_trade(
    ...
    position_params = Open_Position(
        symbol=deps.contract_symbol,
        type=order_type,
        price=decision.get("price") or float(entry_price),
        side=side,
        tp=take_profit,
        sl=stop_loss,
        quantity=quantity,
    ...
    try:
        type_precision = market.get("precision", {}).get("price") if market else None
        enforce_max_loss_sl(
            position_params,
            max_loss_percent=float(os.getenv("MAX_LOSS_PERCENT", "4")),
            price_precision=type_precision,
        )
    except Exception:
        pass
    ...
    if order is None:
        ...
        _record_skip(
            ...
            reason="order_execution_failed",
            decision=decision,
            meta={"params": position_params.dict()},
        )
        return
